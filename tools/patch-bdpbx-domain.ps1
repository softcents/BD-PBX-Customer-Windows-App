...
$cpp = $cpp.Replace('`tTranslateDialog(this->m_hWnd);', '`tSetIcon((HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_MAINFRAME), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR), FALSE);' + [Environment]::NewLine + '`tSetIcon((HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_MAINFRAME), IMAGE_ICON, 32, 32, LR_DEFAULTCOLOR), TRUE);' + [Environment]::NewLine + [Environment]::NewLine + '`tTranslateDialog(this->m_hWnd);')
$cpp = $cpp.Replace('`tON_WM_CREATE()' + [Environment]::NewLine, '`tON_WM_CREATE()' + [Environment]::NewLine + '`tON_WM_CTLCOLOR()' + [Environment]::NewLine)
$ctl = @('HBRUSH AccountDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)','{','`tHBRUSH hbr = CDialog::OnCtlColor(pDC, pWnd, nCtlColor);','`tif (!pWnd) return hbr;','`tconst int id = pWnd->GetDlgCtrlID();','`tif (nCtlColor == CTLCOLOR_STATIC) {','`t`tpDC->SetTextColor((id == IDC_SYSLINK_DISPLAY_PASSWORD) ? RGB(0, 106, 78) : RGB(0, 51, 102));','`t`tpDC->SetBkColor(GetSysColor(COLOR_3DFACE));','`t`treturn GetSysColorBrush(COLOR_3DFACE);','`t}','`tif (nCtlColor == CTLCOLOR_EDIT || nCtlColor == CTLCOLOR_LISTBOX) {','`t`tpDC->SetTextColor(RGB(20, 20, 20));','`t`tpDC->SetBkColor(RGB(255, 255, 255));','`t`treturn GetSysColorBrush(COLOR_WINDOW);','`t}','`tif (nCtlColor == CTLCOLOR_BTN) {','`t`tif (id == IDOK) return (HBRUSH)m_bdGreenBrush.GetSafeHandle();','`t`tif (id == IDCANCEL) return (HBRUSH)m_bdBlueBrush.GetSafeHandle();','`t}','`treturn hbr;','}','') -join [Environment]::NewLine
